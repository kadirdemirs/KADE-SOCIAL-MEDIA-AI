...
import { BRAND_VOICE_SYSTEM_PROMPT, buildBrandVoicePrompt } from '@/lib/ai/prompts'
import { AIModel } from '@/types'
export async function POST(req: NextRequest) {
  try {
    const { name, niche, values, sampleContent, model } = await req.json()
    if (!name || !niche || !model) return NextResponse.json({ error: 'Eksik parametreler' }, { status: 400 })
    const result = await generateContent({ prompt: buildBrandVoicePrompt(name, niche, values || '', sampleContent || ''), model: model as AIModel, systemPrompt: BRAND_VOICE_SYSTEM_PROMPT, maxTokens: 2500 })
