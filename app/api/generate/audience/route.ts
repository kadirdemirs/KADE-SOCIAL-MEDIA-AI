...
import { AUDIENCE_SYSTEM_PROMPT, buildAudiencePrompt } from '@/lib/ai/prompts'
import { AIModel } from '@/types'
export async function POST(req: NextRequest) {
  try {
    const { niche, platform, currentStats, model } = await req.json()
    if (!niche || !model) return NextResponse.json({ error: 'Eksik parametreler' }, { status: 400 })
    const result = await generateContent({ prompt: buildAudiencePrompt(niche, platform || 'youtube', currentStats || ''), model: model as AIModel, systemPrompt: AUDIENCE_SYSTEM_PROMPT, maxTokens: 3000 })
