...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await generateContent({ prompt: buildCoursePrompt(topic, targetAudience || 'yeni başlayanlar', level || 'başlangıç', duration || '5 saat'), model: model as AIModel, systemPrompt: COURSE_SYSTEM_PROMPT, maxTokens: 4000 })
