...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await generateContent({ prompt: buildCoursePrompt(topic, targetAudience || 'yeni baÅŸlayanlar', level || 'baÅŸlangÄ±Ã§', duration || '5 saat'), model: model as AIModel, systemPrompt: COURSE_SYSTEM_PROMPT, maxTokens: 6000 })
