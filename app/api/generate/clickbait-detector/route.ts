...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await generateContent({ prompt: buildClickbaitPrompt(title, platform || 'youtube'), model: model as AIModel, systemPrompt: CLICKBAIT_SYSTEM_PROMPT, maxTokens: 1500 })
