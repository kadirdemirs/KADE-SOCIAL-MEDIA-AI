...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await generateContent({ prompt: buildSponsorScriptPrompt(brand, product, channel || '', duration || '60 saniye', keyPoints || ''), model: model as AIModel, systemPrompt: SPONSOR_SCRIPT_SYSTEM_PROMPT, maxTokens: 2000 })
