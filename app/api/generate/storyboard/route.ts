...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await generateContent({ prompt: buildStoryboardPrompt(script, videoStyle || 'youtube vlog', duration || '5 dakika'), model: model as AIModel, systemPrompt: STORYBOARD_SYSTEM_PROMPT, maxTokens: 4000 })
